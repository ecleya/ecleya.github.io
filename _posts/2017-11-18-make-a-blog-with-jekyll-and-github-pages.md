...
이제 세팅은 모두 끝났습니다. 플러그인도 설치되었으니, 로컬에서 블로그를 한번 띄워볼까요?

```
$ jekyll serve
```

위의 명령을 실행하면 로컬에 블로그 서버가 실행됩니다. 브라우저를 열어서 `https://127.0.0.1:4000`에 접속했을 때 아래와 같은 화면이 나오면 성공입니다.

![성공이다~~]({{ site.url }}/assets/2017-11-18-make-a-blog-with-jekyll-and-github-pages/jekyll_home.png)

Jekyll은 사용자가 입력한 소스코드를 이용해서 정적 웹페이지를 만들어주는 프로그램입니다. 즉, 위와 같이 로컬에서 서버를 띄울 때마다 정적 파일이 어딘가에 생성되게됩니다.
...
`_site`라는 폴더가 보이시죠? `_posts`에 적은 글을 Jekyll이 HTML로 변경한 결과물이 저장되는 폴더가 `_site`입니다. 즉, 웹 호스팅에 사용되는 파일은 모두 `_site`안에 있는 것이죠. 그런데, 이 파일들은 GitHub Pages에서도 만들어집니다. 그래서 `.gitignore`파일을 확인해보시면 `_site`가 추가되어 있는 것을 확인하실 수 있습니다.

## 배포하기
이제 로컬에 블로그를 만들었으니 GitHub Pages에 배포해보겠습니다. 배포는 간단합니다. 지금까지 생성한 모든 내용을 로컬 저장소에 commit하고, 이걸 원격저장소에 push하기만 하면 나머지는 GitHub Pages가 알아서 진행해줍니다. 다음 명령어로 저장소를 push해 보겠습니다.
